...
/**
 * Monotonic state for v7. A batch of identifiers is generated far faster than
 * the clock ticks, so without this every id in a batch shares one millisecond
 * and the trailing random bits decide the order — which is to say, no order at
 * all. RFC 9562 §6.2 "Method 1" fixes it with a counter in the rand_a field.
 */
let lastMillis = -1;
let sequence = 0;
...
/**
 * UUID v7: 48-bit big-endian Unix milliseconds, a 12-bit monotonic counter,
 * then randomness. Sorting the identifiers sorts them by creation time — and
 * with the counter that holds *within* a millisecond too, not just across them.
 */
function uuidV7(): string {
  ...
  if (now > lastMillis) {
    lastMillis = now;
    // Seed randomly, but in the lower half so a large batch cannot overflow.
    sequence = (((bytes[6] & 0x0f) << 8) | bytes[7]) & 0x07ff;
  } else {
    // Same millisecond, or the clock went backwards: step the counter so this
    // id still sorts after the previous one.
    sequence += 1;
    if (sequence > SEQUENCE_MAX) {
      // Counter exhausted — borrow the next millisecond to keep ordering intact.
      lastMillis += 1;
      sequence = 0;
    }
  }
